...
export default function CreateEventPage() {
  ...
  const toggleTag = (tag: string) => {
    setSelectedTags((prev) =>
      prev.includes(tag) ? prev.filter((item) => item !== tag) : selectedTags.length < 5 ? [...prev, tag] : prev
    );
  };
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2 mb-2">
                <Checkbox id="free-event" checked={isFree} onCheckedChange={(checked) => setIsFree(checked === true)} />
                <label
                  htmlFor="free-event"
                  className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                >
            ...
            <div className="space-y-2">
              ...
              <div className="rounded-md border p-4 space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <Badge variant={isFree ? 'outline' : 'secondary'}>{isFree ? 'Free' : `${price || '0'} kr`}</Badge>
                </div>
